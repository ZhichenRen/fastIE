...
class HandshakingKernel(nn.Module):
    ...
    def enc_inner_hiddens(self, seq_hiddens, inner_enc_type='lstm'):
        ...
        def pool(sequence, pooling_type):
            if pooling_type == 'mean_pooling':
                pooling = torch.mean(sequence, dim=-2)
            elif pooling_type == 'max_pooling':
                pooling, _ = torch.max(sequence, dim=-2)
            elif pooling_type == 'mix_pooling':
                pooling = self.lamtha * torch.mean(sequence, dim=-2) + (
                    1 - self.lamtha) * torch.max(sequence, dim=-2)[0]
            return pooling
